...
export async function notifyCandidate(offerId, candidateId, status) {
  ...
  // Busca el email y nombre del candidato en la BD
  ...
  // Arma el payload que le mandamos a n8n
  const payload = {
    candidate_name: candidate.name,
    candidate_email: candidate.email,
    offer_title: offer.title,
    // status puede ser: "technical_test" | "approved" | "rejected"
    status
  };
